...
const App: React.FC = () => {
  ...
  const handleClaimRequest = async (id: string) => {
    ...
    try {
      ...
      console.log(`Claiming request ${id} for ${designerName}`);
      await assignDesignerToRequest(id, designerName);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  {selectedRequest.referenceUrl ? (
                    selectedRequest.referenceUrl.startsWith('data:application/pdf') ? (
                      <button
                        onClick={() => openPdf(selectedRequest.referenceUrl!)}
                        className="w-full aspect-video bg-stone-100 rounded-xl overflow-hidden border hover:border-brand-500 transition-colors flex flex-col items-center justify-center text-brand-600 gap-2 hover:bg-brand-50"
                      >
                        <FileText className="w-12 h-12" />
                        <span className="font-bold text-sm">View PDF Reference</span>
                      </button>
                    ) : (
                        ...
                        <tr key={req.id} onClick={() => setSelectedRequest(req)} className="cursor-pointer hover:bg-brand-50/50 transition-colors group">
                          ...
                          <td className="px-6 py-4" onClick={e => e.stopPropagation()}>
                            <div className="flex gap-2 justify-end">
                              {req.status === 'Pending' && currentUser?.role === 'Designer' && <button onClick={() => handleClaimRequest(req.id)} className="px-3 py-1.5 bg-brand-600 text-white rounded-lg text-xs font-bold hover:bg-brand-700 shadow-sm">Ambil</button>}
