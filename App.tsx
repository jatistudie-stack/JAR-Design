...
const App: React.FC = () => {
  ...
  }, [requests, searchQuery, statusFilter, designerFilter]);
  ...
  }, [requests, historyStartDate, historyEndDate, searchQuery]);
  ...
  const handleClaimRequest = async (id: string) => {
    ...
    if(!confirm("Yakin ingin mengambil job ini? Status akan berubah menjadi In Progress.")) return;
    
    setIsClaiming(id);
    try {
        console.log(`Claiming request ${id} for ${currentUser.name}`);
        await assignDesignerToRequest(id, currentUser.name);
  ...
  const handleRequestSubmit = async (e: React.FormEvent) => {
    ...
    try {
        if (editingRequestId) {
            await updateRequest({ ...formData, id: editingRequestId } as DesignRequest);
        } else {
  ...
  const handleResultFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0] && uploadingRequestId) {
      const file = e.target.files[0];
      try {
        const base64 = await convertToBase64(file);
        await updateRequestResult(uploadingRequestId, file.name, base64);
        await refreshRequests();
      } catch (e) {
          alert("Upload failed.");
      }
  ...
  if (viewState === 'LOGIN') return (
    <div className="min-h-screen flex items-center justify-center bg-stone-100 p-4">
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md border border-stone-200">
        <div className="text-center mb-8"><ChefHat className="w-12 h-12 text-brand-600 mx-auto mb-4" /><h1 className="text-2xl font-bold text-stone-900 tracking-tight">JAR Design Hub</h1><p className="text-stone-500 mt-1">Please sign in to your account</p></div>
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex overflow-hidden">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8">
                    ...
                    <div className="space-y-2"><p className="text-[10px] uppercase font-bold text-stone-400 tracking-widest">Details & Elements</p><div className="bg-stone-50 p-4 rounded-xl text-stone-700 leading-relaxed whitespace-pre-wrap border border-stone-100">{selectedRequest.elements}</div></div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        {selectedRequest.referenceUrl ? (
                          <a href={selectedRequest.referenceUrl} target="_blank" rel="noreferrer" className="block aspect-video bg-stone-100 rounded-xl overflow-hidden border hover:border-brand-500 transition-colors">
                            <img src={selectedRequest.referenceUrl} className="w-full h-full object-cover" alt="Ref" />
                          </a>
                        ) : <div className="aspect-video bg-stone-50 rounded-xl border border-dashed flex items-center justify-center text-stone-400 text-sm">No reference</div>}
                      </div>
                      <div className="space-y-2">
                    ...
                    <div className="flex justify-center pt-4 border-t">
                        {selectedRequest.status === 'Pending' && (currentUser?.role === 'Designer' || currentUser?.role === 'Admin') && (
                            <Button 
                        ...
                        {selectedRequest.status === 'In Progress' && currentUser?.role === 'Designer' && selectedRequest.designerName === currentUser.name && (
                            <Button 
                              onClick={() => {setUploadingRequestId(selectedRequest.id); resultFileInputRef.current?.click();}}
      ...
      <aside className={`fixed md:relative z-30 w-64 h-full bg-white border-r transform transition-transform md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col`}>
        <div className="p-6 border-b flex items-center gap-3"><div className="w-10 h-10 bg-brand-600 rounded-xl flex items-center justify-center text-white"><ChefHat className="w-6 h-6" /></div><span className="font-bold text-lg text-stone-900">JAR Portal</span></div>
            ...
            {activeTab === 'STATUS' && (
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                      <div className="bg-white p-4 rounded-2xl border shadow-sm"><p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">Total</p><p className="text-2xl font-bold text-stone-900">{requests.length}</p></div>
                      <div className="bg-white p-4 rounded-2xl border shadow-sm"><p className="text-[10px] font-bold text-yellow-600 uppercase tracking-widest">Pending</p><p className="text-2xl font-bold text-stone-900">{requests.filter(r => r.status === 'Pending').length}</p></div>
                      <div className="bg-white p-4 rounded-2xl border shadow-sm"><p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">In Progress</p><p className="text-2xl font-bold text-stone-900">{requests.filter(r => r.status === 'In Progress').length}</p></div>
                      <div className="bg-white p-4 rounded-2xl border shadow-sm"><p className="text-[10px] font-bold text-green-600 uppercase tracking-widest">Done</p><p className="text-2xl font-bold text-stone-900">{requests.filter(r => r.status === 'Done').length}</p></div>
                    </div>
                                    ...
                                    <tr key={req.id} onClick={() => setSelectedRequest(req)} className="cursor-pointer hover:bg-brand-50/50 transition-colors group">
                                        ...
                                        <td className="px-6 py-4" onClick={e => e.stopPropagation()}>
                                            <div className="flex gap-2 justify-end">
                                                {req.status === 'Pending' && (currentUser?.role === 'Designer' || currentUser?.role === 'Admin') && <button onClick={() => handleClaimRequest(req.id)} className="px-3 py-1.5 bg-brand-600 text-white rounded-lg text-xs font-bold hover:bg-brand-700 shadow-sm">Ambil</button>}
                                                {req.status === 'In Progress' && currentUser?.role === 'Designer' && req.designerName === currentUser.name && <button onClick={() => {setUploadingRequestId(req.id); resultFileInputRef.current?.click();}} className="px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs font-bold hover:bg-green-700">Finish</button>}
